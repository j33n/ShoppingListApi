...
class ApiTestCase(unittest.TestCase):
    ...
    def test_create_shoppinglist(self):
        ...
        self.assertTrue(
            "No shoppinglists found here, please add them.", response1.data)
    ...
    def test_invalid_shoppinglists(self):
        ...
        self.assertIn(b'Title can\'t be numbers only', response1.data)
        self.assertEqual(400, response1.status_code)
        self.assertIn(
            b'Your title should be more than 6 characters', response2.data)
        self.assertEqual(400, response2.status_code)
        self.assertIn(b'Your title is empty', response3.data)
    ...
    def test_update_shoppinglist(self):
        ...
        self.assertTrue(b'fail' in duplicate_update.data)
        self.assertTrue(
            b'Shopping List my favorite shoes already exist'
        ...
        self.assertTrue(
            b"Shopping List my favorite shoes already exists"
            in response1.data
        )
        self.assertEqual(400, response1.status_code)
        ...
        self.assertTrue(
            b"Shopping List my favorite shoes already exists"
            in check_update.data
        )
        self.assertEqual(400, check_update.status_code)
        ...
        self.assertIn(b"Title can\'t be numbers only",
                      check_wrong_update_1.data)
        self.assertEqual(400, check_wrong_update_1.status_code)
        self.assertIn(b"Your title should be more than 6 characters",
                      check_wrong_update_2.data)
        self.assertEqual(400, check_wrong_update_2.status_code)
        self.assertIn(b"Title can\'t be numbers only",
                      check_wrong_update_3.data)
        self.assertIn(b"Title can\'t be numbers only",
                      check_wrong_update_3.data)
        self.assertEqual(400, check_wrong_update_3.status_code)
        self.assertIn(
            b"Your description should be more than 6 characters",
            check_wrong_update_4.data
        )
        self.assertEqual(400, check_wrong_update_4.status_code)
    ...
    def test_invalid_data_update(self):
        ...
        self.assertEqual(400, response3.status_code)
        self.assertIn(
            b'Your title should be more than 6 characters', response4.data)
        self.assertEqual(400, response4.status_code)
        self.assertIn(b'Your description is empty', response5.data)
    ...
    def test_add_shoppinglistitem(self):
        ...
        self.assertTrue(
            b"You don\'t have any items for now" in check_emptyness.data)
        self.assertEqual(400, check_emptyness.status_code)
        ...
        self.assertIn(b'Your item_title is empty', create_invalid_item.data)
        self.assertEqual(400, create_invalid_item.status_code)
        ...
        self.assertIn(b'Item_Description can\'t be numbers only',
                      create_invalid_item_2.data)
        self.assertEqual(400, create_invalid_item_2.status_code)
        ...
        self.assertIn(
            b'Your item_description should be more than 6 characters',
            create_invalid_item_2.data
        )
        self.assertIn(b'Item_Description can\'t be numbers only',
                      create_invalid_item_2.data)
    ...
    def test_invalid_item_value(self):
        ...
        self.assertIn(b'Item_Title can\'t be numbers only', response1.data)
        self.assertEqual(400, response1.status_code)
        self.assertIn(
        ...
        self.assertEqual(400, response2.status_code)
        self.assertIn(b'Your item_title is empty', response3.data)
        self.assertEqual(400, response3.status_code)
    ...
    def test_fetch_shoppinglist_item(self):
        ...
        get_single_item = self.client().get(
            '/shoppinglist/{0}/item/{1}'.format(
                results['id'], results1['item_id']),
            headers=dict(Authorization=access_token)
        )
        ...
        create_duplicate_item = self.client().post(
            ...
            headers=dict(Authorization=access_token),
            data=self.shoppinglistitem)
        self.assertEqual(create_duplicate_item.status_code, 400)
        self.assertIn(b'Shopping List item vegetables already exists',
                      create_duplicate_item.data)
        # Test non existing items
        get_wrong_item = self.client().get(
            '/shoppinglist/{0}/item/3'.format(
                results['id'], results1['item_id']),
            headers=dict(Authorization=access_token)
        )
        self.assertIn('Requested value \\\'3\\\' was not found',
                      str(get_wrong_item.data))
        self.assertEqual(get_wrong_item.status_code, 500)
    ...
    def test_update_shoppinglistitem(self):
        ...
        self.assertTrue(
            b"Shopping list item updated successfuly" in update_resp.data)
        self.assertEqual(update_resp.status_code, 200)
        ...
        self.assertTrue(
            b"Shopping list item sausages and stuff already exists"
            in non_existing_updates.data
        )
        self.assertEqual(non_existing_updates.status_code, 400)
    ...
    def test_invalid_item_update(self):
        ...
        self.assertEqual(400, response2.status_code)
        self.assertIn(b'Your item_description is empty', response3.data)
        self.assertEqual(400, response3.status_code)
        self.assertIn(b'Item_Title can\'t be numbers only', response4.data)
        self.assertIn(
            b'Your item_title should be more than 6 characters',
            response4.data
        )
    ...
    def test_delete_wrong_shoppinglistitem(self):
        ...
        self.assertTrue(
            b"Requested value '7' was not found" in delete_item.data)
    ...
    def test_token_unprovided(self):
        ...
        for url in url_list_get:
            ...
            self.assertEqual(response.status_code, 500)
        url_list_post = ['/shoppinglists',
                         '/shoppinglist/1/items', 'auth/logout']
    ...
    def test_question(self):
        ...
        self.assertIn(b"User account created successfuly",
        ...
        self.assertIn(b"Answer can\'t be numbers only",
                      create_invalid_question.data)
        self.assertIn(b"Your answer should be more than 6 characters",
                      create_invalid_question.data)
    ...
    def test_reset_password(self):
        ...
        self.assertIn("User account created successfuly",
                      str(register_user.data))
        access_token = self.access_token()
        ...
        self.assertIn(b'Your question is not valid',
                      invalid_security_question.data)
    ...
    def test_update_accountinfo(self):
        ...
        wrong_password_userupdate = self.client().put(
            '/user',
            headers=dict(Authorization=access_token),
            data={'new_email': 'johndoe@test.com',
                  'new_username': 'John Doe',
                  'password': 'my_wrong_password'})
        self.assertEqual(400, wrong_password_userupdate.status_code)
        self.assertIn(b"You need your password to update account info.",
                      wrong_password_userupdate.data)
        ...
        self.assertEqual(400, update_user_1.status_code)
        self.assertIn(
            b"Your new_email should be more than 6 characters",
            update_user_1.data)
        # check invalid new username
        ...
        self.assertIn(b"Your new_email is not valid", update_user_2.data)
        self.assertEquals(400, update_user_2.status_code)
        # check email and username were updated
        check_update = self.client().get(
            '/user',
            headers=dict(Authorization=access_token)
        )
    ...
    def test_paginated_shoppinglist(self):
        ...
        self.register_user()
        access_token = self.access_token()
        no_shoppinglists = self.client().get(
            '/shoppinglists/1',
            headers=dict(Authorization=access_token)
        )
        self.assertIn(
            b'No shoppinglists found here, please add them.',
            no_shoppinglists.data
        )
        ...
        self.assertIn(b'Shopping List created successfuly',
                      add_shoppinglist_1.data)
        self.assertEqual(200, add_shoppinglist_1.status_code)
        empty_page = self.client().get(
            '/shoppinglists/2', headers=dict(Authorization=access_token))
        self.assertIn(
            b'No shoppinglists found here, please add them.', empty_page.data)
        self.assertEqual(200, empty_page.status_code)
        page_with_shoppinglist = self.client().get(
            '/shoppinglists/1', headers=dict(Authorization=access_token))
        self.assertIn(b'Items to cook my favorite meal',
                      page_with_shoppinglist.data)
    ...
    def test_search_queries(self):
        ...
        self.assertEqual(200, add_shoppinglist.status_code)
        self.assertIn(b'Shopping List created successfuly',
                      add_shoppinglist.data)
        response = self.client().get(
            '/search/q=meal',
            headers=dict(Authorization=access_token)
        )
        ...
        not_found_search = self.client().get(
            '/search/q=random_search',
            headers=dict(Authorization=access_token)
        )
        ...
        mess_up_token = access_token + "Mess up token"
        chech_authorization = self.client().get(
            '/search/q=meal', headers=dict(Authorization=mess_up_token))
        self.assertTrue(401, chech_authorization.status_code)
        self.assertIn(b'Invalid token. Please log in again.',
                      chech_authorization.data)
